...
# '<our_project_root>/config'. This allows SCOTER to give the paths below
# relative to '<our_project_root>'.
path_prefix: '..'
...
dataset_config: !gp.DatasetConfig
  ...
  # include the placeholder '${event_name}'.
  ...
  # Path and file root name (no extension) for input travel-time grid files
  # (format -> NonLinLoc 3D grid file)
  traveltimes_path: 'time/fescan'

  # Files with pre-computed takeoff angles (SPTree interpolators) (optional).
  # Given template path must include the placeholder '${phase_label}'
  ...
  # a template path that includes the placeholder '${event_name}' and makes
...
station_terms_config: !gp.StationTermsConfig
  ...
  static_config: !gp.StaticConfig
    ...
    phase_list: ['P1', 'Pg', 'Pb', 'Pn', 'P', 'S1', 'Sg', 'Sb', 'Sn', 'S']
  ...
  locqual_config: !gp.LocationQualityConfig
    ...
    # Events with low-quality locations (i.e. do not satisfy one or more of the
    # following conditions) are not used in station terms calculation.
